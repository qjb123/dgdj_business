...
abstract class CalendarPagerView extends ViewGroup implements View.OnClickListener {
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        final int measureTileSize = specWidthSize / DEFAULT_DAYS_IN_WEEK;//我没懂你的意思,你是要整个要
        final int measureTitleHeight = specHeightSize/8;//这个有可能是7行,先按8行算吧.
        final int minMeasureTitleSize = Math.min(measureTileSize, measureTitleHeight);
        System.out.println("measureTitleSize:"+measureTileSize+"  measureTitleHeight:"+measureTitleHeight+"  minMeasureTitleSize"+minMeasureTitleSize);
        ...
        for (int i = 0; i < count; i++) {
            ...
            child.measure(childWidthMeasureSpec, childHeightMeasureSpec);//宽和高是一样的啊
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        final int perViewWidth = (getWidth()-getPaddingLeft()-getPaddingRight())/7;//计算每个控件的应该占有的宽度
        final int perViewHeight = (getHeight()-getPaddingTop()-getPaddingBottom())/8;//计算每个控件的应该占有的高度
        int childRaw = 0;//子控件是第几行
        int childLeft = 0;//每个子控件最左边的位置
        for (int i = 0; i < count; i++) {
            final View child = getChildAt(i);

            final int width = child.getMeasuredWidth();
            final int height = child.getMeasuredHeight();
            childLeft = perViewWidth * (i%7) + (perViewWidth - width) / 2;
            int childTop = perViewHeight*childRaw+ (perViewHeight - width) / 2;
            child.layout(childLeft,childTop , childLeft + width, childTop + height);
            Log.d("gtt", "onLayout: "+childLeft);
